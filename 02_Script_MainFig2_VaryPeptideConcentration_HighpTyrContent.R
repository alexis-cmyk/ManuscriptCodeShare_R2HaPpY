...
## --------------------------------------------------------------------------------------------------------------------------------------------------
human_fasta_2024 <- read_csv("raw_data/MainFig2/sp_reviewed_UP000005640_2024_03_21_forR.csv")


haloTEVsSrc <- read_csv("raw_data/MainFig2/haloTEVsSrc_forR.csv") %>%
  clean_names() %>% 
  mutate(
...
Ecoli_fasta_2017 <- read_csv("raw_data/MainFig2/Ecoli_K12_uniprot_April2017_BR_forR.csv") %>%
  clean_names() %>% 
  mutate(
    organism = "E.coli",
    length = str_length(full_protein_sequence),
...
comet <- read_csv("raw_data/MainFig2/comet/result.csv") %>%
  clean_names() %>%
...
## --------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  ascore <- read_csv("raw_data/MainFig2/ascore/result.csv") %>%
  clean_names() %>%
  separate(sample_name, into = c("exp", "lysate", "well", "pept_amt","condition", "inj_vol", "rep"), sep = "_", remove = FALSE) %>% 
  filter(lysate == "HFPV") %>%
  filter(well != "pool") %>%
