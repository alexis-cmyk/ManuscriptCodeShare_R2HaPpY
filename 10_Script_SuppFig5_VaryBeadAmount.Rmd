...
##_c) your data
###__i) comet
Z:\Git clones\AC077_HaloBeadQC\raw_data\HiResAscore\AC077_comet_hiResAscore
```{r}
...
###__i) ascore
Z:\Git clones\AC077_HaloBeadQC\raw_data\HiResAscore\AC077_ascore_hiResAscore
```{r}
...
```
label positions for replicate pY site plot.
```{r}
...
###__i) factor key
 -- Here the factors are psites, 
 -- but keys will be any observation type the PCA is applied to. e.g. p-sites, peptides, proteins, etc.....
```{r}
...
###__iv) extract factors for painting
```{r}

```


##_g) differential abundance
df
```{r}
...
###__ii) paint PCA loadings
```{r}

```

#5. PTM SEA 
##_a) data prep for input
```{r}
