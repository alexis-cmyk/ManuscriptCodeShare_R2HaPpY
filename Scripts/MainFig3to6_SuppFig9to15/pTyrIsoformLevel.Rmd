...
comet <- read_csv("raw_data/MainFig3to6_SuppFig9to15/TyrPhospho/20241001_final_comet/result.csv") %>%
  clean_names() %>% 
...
```{r}
ascore <- read_csv("raw_data/MainFig3to6_SuppFig9to15/TyrPhospho/20241001_final_ascore/result.csv") %>% 
...
##__b) pS/pT shared with pY
we observed only 66% of phosphosites in site level data were on pY. This was quite low. To get sense whether sSrc is enriching pS or pT sites on their own or if sSrc is pulling down pY that also have pS or pT, I will see how many pS or pT sites are on the same vs. different peptides as a pY site.
...
n_psites_per_isoform_df <- ascore_fasta_protein_mod_loc_sum_isoform %>% 
  ...
  ungroup()
```
plot distribution of isoform types
```{r}
...
```{r}
# write_csv(x = ascore_final_gene %>% mutate(gene_ref = paste(gene, isoform_cluster, sep = "_")), file = "modified_data/DDA_YP/isoform_SHINY_input.csv")
```

```{r}
most_common_protein_family_per_site <- read_csv("modified_data/DDA_YP/EGF_stim/noPOOL/min_3reps_once/PUB/KEY_most_common_protein_family_per_site.csv")
```
...
write_csv(x = SHINY_pY_isoforms, file = "modified_data/DDA_YP/pY_ISOFORM_input.csv")
```
...
```
PC3 seems to correlate with MS run order more than biology.
...
# 
# write_csv(x = PTMNav_isoform_input, file = "modified_data/DDA_YP/PTMNav_isoform_input.csv")
```
