...
fasta_gene_names <- read_tsv(file = "raw_data/uniprotkb_proteome_UP000005640_2024_09_04.tsv") %>%
  clean_names() %>% 
...
fasta_subcellular_location <- read_tsv("raw_data/uniprotkb_proteome_UP000005640_2024_10_16_locations.txt") %>% clean_names() %>% 
...
##PTMSEA
```{r}
EGFR1_pathway <- read_csv("raw_data/EGFR1_phosphopathway_PTMSEA.csv") %>% clean_names() %>%
...
##Phosphopedia
```{r}
ppdia_ErbB_signaling <- read_csv(file = "raw_data/Phosphopedia_ErbB_gene_sites.csv") %>% 
  clean_names()
```
...
##_a. PSP all p-sites
```{r}
# PSP_all <- read_csv(file = "raw_data/Phosphorylation_site_dataset/Phosphorylation_site_dataset.csv") %>% clean_names() %>%
...
# write_csv(x = PSP_all, file = "raw_data/PSP_all_psites.csv")

PSP_all <- read_csv ("raw_data/PSP_all_psites.csv")
```
...
```{r}
EGFR1_pathway_PTMSigDB <- read_csv("raw_data/EGFR1_phosphopathway_PTMSEA.csv") %>% clean_names() %>%
...
```{r}
PSP_EGFR <- read_csv(file = "raw_data/PSP_EGF_all_20241219.csv") %>% 
  clean_names() %>% 
...
```{r}
WP437_EGFR_node_depths <- read_csv(file = "raw_data/EGFR_WP437_node_depths.csv") %>%  clean_names()
```
...
####--filter for confidently localized phosphosites
...
##rollup PSM to precursor, then sum precursor intensities to p-site and median normalize
...
```
perhaps it is the case that some pY sites increase in abundance enough towards 15 min stimulation to appear as abundance increase.

### b. LIST OF pSTY SITES FOR CORAL
get total pSTY sites per uniprot ID to map in coral. regulated is not a requirement.
...
#location along protein sequence
```{r}
...
###__i). gene sites unique to condition
```{r}
...
```
here is a more dialed representation of present vs. not present proteins and associated psite refrencesby removing duplicate observed sites for different reference sites.

Notably, I am missing many pY sites with global phospho enrichment. I would like to plot the number of pY sites in ErbB2 signaling that I do vs. don't detect (relative to annotated reference sites) relative to number of pST sites I do and don't detect relative to reference sites. 
Also, I would like to plot overlap of sites in ErbB2 and EGFR and Pi3k signaling to better understand the variation in p-sites among reference pathways. Also look in PSP and commercial phospho specific antibodies marketed for tracing ErbB and EGFR signaling. These antibodies will be wayyy more expensive than global and SB enrichment if I can show they enrich sites reproducibly and sensitively.
...
```
median CV of 23 - 27%CV is quite good for phospho across 6 replicates.
this is at site level, not phosphoisoform level. 
Also, precursor intensities were summed to phosphosite level.
...
#####-----overall percent p-sites changing per cell type
```{r}
...
```
So I see the EGFR pY sites, but they show minimal fold change. 
Make boxplots with points to see if minimal fold change is due to variation in intensity or low signal relative to noise, or in fact minimal change in intensity of those EGFR sites.
...
###---extract most common redundancy per site
```{r}
...
##_d) function to iteratre through my data
```{r}
...
#5.5 FUNCTION from pY script to iterate through plots
...
##--pSTY sites in EGFR annotated pathway from PTMSEA
```{r}
...
write_csv(x = PTMNav_psite_input, file = "modified_data/DDA_GP/PTMNav_psite_input.csv")
```


#---------------------------------------------
#7. SUBCELLULAR LOCATIONS
...
#------------------------------------------------------
#------------------------------------------------------
# _***imported from pY site script for temporal response profiling***_
#------------------------------------------------------
#------------------------------------------------------
...
11/24/2024 --> I need to figure out why I don't use +SD imputation for heatmap but +SD imputation for fold change analysis. I think the imputation should be the same across the board.
...
```
defunct analysis because I impute for any thing in 50% of at least one condition, regardless of presence in other conditions.

#--------------------------------------
#--------------------------------------

# 16. FOLD CHANGE IMPUTED
##__a. dataframe
...
###__i. total regulated p-sites after imputation
```{r}
...
```
###___iv. painted by DB annotations
```{r}
...
####____List of regulated pY sites from imputation with SD
...
```
4 or 5 clusters seems reasonable if considering only amount of variation explained per k-means method. Next visually see how much variation is explained by overall trends.



###_c. actual clustering function call
...
## _d .HEATMAP ordered by clusters (mean w/o SD imputed + zscored)

####___i. without data completeness summary
##### . . . database comparison at gene level
```{r}
set.seed(15)
...
NOTE: SOMETHING SEEMS OFF FOR PSTY ADAPTION OF CLUSTERING WITH MATCHES TO GENE LEVEL. THE EXACT SAME PATTERN OF Z-SCORE FOR GENES MATCHED AND UNMATCHED TO EXTERNAL DATABASES. i THINK CODE ISSUE IS LIKELY.

##### . . . database comparison at site level
```{r}
set.seed(15)
