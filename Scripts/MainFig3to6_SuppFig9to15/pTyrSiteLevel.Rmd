...
```{r}
ascore_redundancyinfo <- read_csv("raw_data/MainFig3to6_SuppFig9to15/TyrPhospho/AC080_ascore_w_proteins_andPOOL/result.csv") %>% 
...
##_d. ascore parsing
```{r}
ascore <- read_csv("raw_data/MainFig3to6_SuppFig9to15/20241001_final_ascore/result.csv") %>% 
...
write_csv(SHINY_pY_SITE_input, "modified_data/MainFig3to6_SuppFig9to15/pTyrSiteLevel/shiny_input/SHINY_pY_SITE_input.csv" )
```
...
#### . . .  . write to csv
```{r}
write_csv(x = psite_diff_abundance_gene_noImputation_2xDB, file = "modified_data/MainFig3to6_SuppFig9to15/pTyrSiteLevel/noPOOL/min_3reps_once/psite_diff_abundance_gene_noImputation_2xDB.csv")
```
...
DB_overlap <- psite_diff_abundance_gene_2xDB_imputed_forvolcano %>%
  ...
  rename(position = mod_residue) %>% 
  # rename(acc = reference) %>% 
  rename(residue = mod_res) %>% rbind(DB_overlap_DBonly)
...
#   mutate(
#     num_regulated = case_when(
#       regulated_psite == "regulated" ~ 1,
#       regulated_psite == "not regulated" ~ 0,
#       TRUE ~ 0)) %>% 
#   group_by(reference, position, residue, gene_ref) %>% 
#   mutate(
#     sum_regulated = sum(num_regulated),
#     once_regulated = case_when(
#       sum_regulated > 0 ~ "regulated at least once",
#       sum_regulated == 0 ~ "not at all regulated" ),
#     mod_type_DB = paste(once_regulated, overlap_my_data, sep = "_")) %>% 
#   ungroup()
# #use 'once_regulated' to indicate if site is consdered regulated, in at least one time point relative to untreated.
#   
...
changing_proteins_TP_df <- read_csv("modified_data/MainFig3to6_SuppFig9to15/DDA_TP/proteins_diff_abundance_less.csv") %>% 
...
gene_3xDB_overlap_upset_df <-  PSP_vs_PTMSigDB_EGF %>% 
  ...
  # filter(present_in_DB == TRUE) %>%
    mutate(PSP_upset = case_when(
      ...
      WP437 == TRUE ~ 1, 
      TRUE ~ 0)) %>% 
  select(gene, PSP_upset, PTM_SEA_upset,  WP437_upset) %>% 
  rename(PSP = PSP_upset) %>% 
  rename(PTM_SEA =  PTM_SEA_upset) %>% 
  # rename(PPDIA= PPDIA_upset) %>% 
  rename(WP437 = WP437_upset) %>% 
  select(PSP, PTM_SEA, WP437) %>% 
  mutate_all(.funs = as.numeric) %>% 
  as.matrix() %>% 
  as.data.frame()
...
pdf("bits_motif_thisstudy_vs_both_vs_DBs.pdf", width = 7, height = 8) # Open a new pdf file
gridExtra::grid.arrange(logo_this_study_noY, logo_both_noY, logo_DBs_only_noY) # Write the grid.arrange in the file
dev.off() # Close the file


ggsave("bits_motif_thisstudy_vs_both_vs_DBs.png", p, width = 4, height = 12)
```
...
```{r}
combined_motifs <- read_csv("modified_data/MainFig3to6_SuppFig9to15/pTyrSiteLevel/PSP motif analysis tool/this study vs shared with databases/combined_motifs.csv") %>%
  clean_names() %>%
...
ggsave(filename = "modified_data/MainFig3to6_SuppFig9to15/pTyrSiteLevel/PSP motif analysis tool/this study vs shared with databases/DBs only pY EGF sites/combined_motifs_ratiofound_option4.png", plot = barplot_motifs4, width = 8, height = 7, scale = 0.8)

ggsave(filename = "modified_data/MainFig3to6_SuppFig9to15/pTyrSiteLevel/PSP motif analysis tool/this study vs shared with databases/DBs only pY EGF sites/combined_motifs_ratiofound_option4.pdf", plot = barplot_motifs4, width = 9, height = 6, scale = 0.8)
```
...
write.csv(x = row_annotations, file = "modified_data/MainFig3to6_SuppFig9to15/pTyrSiteLevel/shiny_input/heatmap_row_annotations.csv")
```
...
#write csv for shiny--------------------------------------------------------------
write_csv(x = zscore_cmeans_clustering_matrix_reps_ordered_df1, file = "modified_data/MainFig3to6_SuppFig9to15/pTyrSiteLevel/shiny_input/heatmap_df.csv")
#---------------------------------------------------------------------------------
...
#save for shiny
write.csv(zscore_cmeans_clustering_matrix_reps_ordered_matrix, file = "modified_data/MainFig3to6_SuppFig9to15/pTyrSiteLevel/shiny_input/zscore_cmeans_clustering_matrix_reps_ordered_matrix.csv")
...
write_csv(ascore_gene_clusters_Imputation_noSD, file = "modified_data/MainFig3to6_SuppFig9to15/pTyrSiteLevel/shiny_input/boxplot_input_dataframe.csv")
```
...
#                              dataframe = ascore_gene_clusters_Imputation_noSD %>% distinct(gene, gene_ref, condition, replicate, sample_id, median_norm_intensity, cluster, n_obs) %>% mutate(condition = fct_relevel(condition, "EGF0min", "EGF1min", "EGF3min", "EGF5min", "EGF15min")), output_dir = "output/MainFig3to6_SuppFig9to15/pTyrSiteLevel/EGF_pathway/", strip_color = "pink")
...
ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrSiteLevel/EGF_pathway/pheatmap_imputed_zscore_EGFR_condition_CST_gene_noSD.pdf", plot = zscore_EGFR_pheatmap_condition_noSD_CST , width = 12, height = 60, scale =0.4)

ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrSiteLevel/EGF_pathway/pheatmap_imputed_zscore_EGFR_condition_CST_gene_noSD.png", plot = zscore_EGFR_pheatmap_condition_noSD_CST , width = 5, height = 40, scale = 0.5, limitsize = TRUE)
```
