...
comet <- read_csv("raw_data/MainFig3to6_SuppFig9to15/TyrPhospho/comet/result.csv") %>%
  ...
  filter(grepl("0", replicate) == FALSE) %>% 
  mutate(sample_id = paste(condition, replicate, sep = " "))


## --------------------------------------------------------------------------------------------------------------------------------------------------
ascore_redundancyinfo <- read_csv("raw_data/MainFig3to6_SuppFig9to15/TyrPhospho/ascore_redundancy_info.csv") %>% 
  distinct(ascore_sequence, proteins, gene_redundancy)
...
ascore.site2 <- ascore %>% 
  ...
  select(sample_name, condition, replicate, sample_id,  reference, gene,
         ...
         missed_cleavages, redundancy, full_protein_sequence, organism, protein_names, length) %>%
...
ascore_fasta_protein_mod_loc_sum_isoform <- ascore_fasta_protein_mod_loc %>% 
  ...
  ungroup() %>% 
  distinct(condition, replicate, sample_id, reference, ascore_sequence, mod_protein_location, intensity, proteins, gene_redundancy, redundancy) %>% 
  ...
  pivot_wider(id_cols = c("condition", "replicate", "sample_id", "reference", "ascore_sequence", "intensity", "proteins", "gene_redundancy", "redundancy"),
  ...
  group_by(condition, replicate,  sample_id, reference, isoform_cluster, proteins, gene_redundancy, redundancy) %>%
  ...
  distinct(condition, replicate, sample_id, reference, isoform_cluster, sum_intensity_precursor_to_peptide, log2_peptide_qty, proteins, gene_redundancy, redundancy) %>% 
  ...
  distinct(condition, replicate, sample_id, reference, isoform_cluster, sum_intensity_precursor_to_peptide, median_norm_intensity, raw_median_norm_intensity, proteins, gene_redundancy, redundancy) %>% 
...
ggsave("output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/phospho_pept_counts_noAscore.png", plot = plot_ppept_detections, width = 10, height = 10, scale = 0.5)
ggsave("output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/phospho_pept_counts_noAscore.pdf", plot = plot_ppept_detections, width = 10, height = 10, scale = 0.5)
...
ggsave("output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/phospho_pept_enrich_efficiency_counts.png", plot = plot_ppept_enrich_efficiency, width = 6, height = 8, scale = 0.5)
ggsave("output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/phospho_pept_enrich_efficiency_counts.pdf", plot = plot_ppept_enrich_efficiency, width = 6, height = 9, scale = 0.5)
...
ggsave("output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/phospho_pept_intensity_ratio.png", plot = plot_ratio_phos_intensity, width = 6, height = 8, scale = 0.4)
ggsave("output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/phospho_pept_intensity_ratio.pdf", plot = plot_ratio_phos_intensity, width = 6, height = 9, scale = 0.5)
...
ggsave("output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/all_psm_intensity_distribution_reps.png", plot = plot_psm_intensity_distribution_reps, width = 16, height = 16, scale = 0.4)
ggsave("output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/all_psm_intensity_distribution_reps.pdf", plot = plot_psm_intensity_distribution_reps, width = 16, height = 16, scale = 0.4)
...
# 
# 
# ggsave("output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/plot_psm_intensity_distribution_reps.png", plot = plot_psm_intensity_distribution_reps, width = 10, height = 10, scale = 0.4)
# ggsave("output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/plot_psm_intensity_distribution_reps.pdf", plot = plot_psm_intensity_distribution_reps, width = 10, height = 10, scale = 0.4)


## --------------------------------------------------------------------------------------------------------------------------------------------------
distinct_p_isoforms_total <- ascore_fasta_protein_mod_loc_sum_isoform %>% 
  distinct(reference, isoform_cluster) %>% 
  summarize(
    n_isoforms = n()  )

distinct_p_isoforms_total
...
## --------------------------------------------------------------------------------------------------------------------------------------------------
write_csv(x = distinct_p_isoforms_condition, file = "modified_data/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/pSTYisoforms_ascore13_condition.csv", col_names = TRUE)
...
ggsave("output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/distinct_p_isoforms_condition.png", plot = plot_distinct_p_isoforms_condition, width = 8, height = 8, scale = 0.4)
...
## --------------------------------------------------------------------------------------------------------------------------------------------------
write_csv(x = distinct_p_isoforms_reps, file = "modified_data/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/pSTYisoforms_ascore13_reps.csv", col_names = TRUE)
...
ggsave("output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/distinct_p_isoforms_reps.png", plot = plot_distinct_p_isoforms_reps, width = 20, height = 10, scale = 0.4)
ggsave("output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/distinct_p_isoforms_reps.pdf", plot = plot_distinct_p_isoforms_reps, width = 20, height = 12, scale = 0.4)
...
## --------------------------------------------------------------------------------------------------------------------------------------------------
n_psites_per_isoform_df <- ascore_fasta_protein_mod_loc_sum_isoform %>% 
  separate(isoform_cluster, into = c("psite1", "psite2", "psite3"), sep = " ", remove = FALSE,) %>% 
  mutate(n_psites_per_isoform = str_count(isoform_cluster, pattern = "[STY]"),
         mod_res1 = str_sub(psite1, start = 1L,end = 1L),
         mod_res2 = str_sub(psite2, start = 1L,end = 1L),
         mod_res3 = str_sub(psite3, start = 1L,end = 1L), 
         combined_modres = paste(mod_res1, mod_res2, mod_res3, sep = ""),
         combined_modres = str_replace_all(combined_modres, "NA", "")) %>%
  mutate(sample_id = paste(condition, replicate, sep = " ")) %>% 
  group_by(sample_id) %>% 
  mutate(total_isoforms = n()) %>% 
  ungroup() %>% 
  group_by(sample_id, combined_modres) %>% 
  mutate(total_isoform_combos = n(),
         percent_isoform_combo = total_isoform_combos / total_isoforms * 100) %>% 
  ungroup()


## --------------------------------------------------------------------------------------------------------------------------------------------------
plot_isoform_combos <- ggplot(data = n_psites_per_isoform_df) +
  geom_bar(mapping = aes(x = combined_modres)) +
  alexis_theme()
plot_isoform_combos
...
plot_isoform_combos_pcts <- ggplot(data = (n_psites_per_isoform_df %>% distinct(sample_id, combined_modres, percent_isoform_combo) %>% 
                                     mutate(combined_modres = fct_relevel(combined_modres, "S", "SS", "SSS", "SST", "SSY", "ST", "STS", "STY", "SY", "SYS", "T", "TS", "TSS", "TST", "TSY", "TT", "TTT", "TY", "TYY", "YS", "YSS", "YST", "YT", "YTY", "YY", "YYT", "Y")))) +
  geom_col(mapping = aes(x = sample_id, y = percent_isoform_combo, fill = combined_modres), position = "stack") +
  alexis_theme() +
  scale_fill_viridis_d()
plot_isoform_combos_pcts



# another view
plot_isoform_combos_pcts2 <- ggplot(data = (n_psites_per_isoform_df %>% distinct(sample_id, combined_modres, percent_isoform_combo) )) +
  geom_col(mapping = aes(x = combined_modres, y = percent_isoform_combo, fill = sample_id), position = "dodge") +
  alexis_theme() +
  scale_fill_viridis_d()
plot_isoform_combos_pcts2


#without S, T, or Y sites alone
plot_isoform_combos_pcts2 <- ggplot(data = (n_psites_per_isoform_df %>% distinct(sample_id, combined_modres, percent_isoform_combo) %>%  filter(!combined_modres %in% c("S", "T", "Y")) )) +
  geom_col(mapping = aes(x = combined_modres, y = percent_isoform_combo, fill = sample_id), position = "dodge") +
  alexis_theme() +
  scale_fill_viridis_d()
plot_isoform_combos_pcts2

#distribution of p-sites per isoform
plot_distribution_n_psites_per_isoform <-  ggplot(data = n_psites_per_isoform_df) +
  geom_bar(mapping = aes(x = sample_id, fill = as.factor(n_psites_per_isoform)), position = "fill") + 
  alexis_theme() + 
  scale_fill_viridis_d(option = "C") +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 1), breaks = c(seq(0, 1, 0.1)))

plot_distribution_n_psites_per_isoform
...
png("output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/upset_plot_reps.png", width = 1000, height = 600)
print(upset_plot_reps)
dev.off()


pdf("output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/upset_plot_reps.pdf", width = 12, height = 8)
print(upset_plot_reps)
dev.off()
...
png("output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/upset_plot_condition.png", width = 1000, height = 600)
print(upset_plot_condition)
dev.off()


pdf("output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/upset_plot_condition.pdf", width = 12, height = 8)
print(upset_plot_condition)
dev.off()
...
png("output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/upset_plot_pisoforms_gt3reps.png", width = 2000, height = 600)
print(upset_plot_pisoforms_gt3reps)
dev.off()


pdf("output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/upset_plot_pisoforms_gt3reps.pdf", width = 12, height = 8)
print(upset_plot_pisoforms_gt3reps)
dev.off()
...
# 
# ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/ErbB2_pprotein_heatmap.png", plot = ErbB2_pprotein_heatmap, height = 40, width = 15, scale = 0.3)
...
ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/intensity_distribution_isoform_overlaps.png", plot = intensity_distribution_isoform_overlaps, width = 16, height = 14, scale = 0.4)
...
ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/all_isoform_CVs_plotNotNorm.png", plot = all_isoform_CVs_plot, width = 18, height = 14, scale = 0.4)
...
ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/all_isoform_CVs_plot_Norm.png", plot = all_isoform_CVs_plot_NORM, width = 18, height = 14, scale = 0.4)
...
ggsave("output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/check_all_isoform_intensity_before_normalization.pdf", plot = check_all_isoform_intensity_before_normalization, width = 15, height = 15, scale = 0.5)

ggsave("output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/check_all_isoform_intensity_before_normalization.png", plot = check_all_isoform_intensity_before_normalization, width = 15, height = 15, scale = 0.5)
...
ggsave("output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/check_all_isoform_intensity_after_normalization.pdf", plot = check_all_isoform_intensity_after_normalization, width = 15, height = 15, scale = 0.5)

ggsave("output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/check_all_isoform_intensity_after_normalization.png", plot = check_all_isoform_intensity_after_normalization, width = 15, height = 15, scale = 0.5)
...
ggsave("output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/all_isoform_correlation.png", plot = all_isoform_correlation, width = 20, height = 20, scale = 0.4)
ggsave("output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/all_isoform_correlation.pdf", plot = all_isoform_correlation, width = 16, height = 12, scale = 0.4)
...
isoform_volcano
ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/isoform_volcano.png", plot = isoform_volcano, width = 30, height = 18, scale = 0.4)
...
# 
# ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/my_isoform_volcano_plot_EGF0min_vs_15min.png", plot = my_isoform_volcano_plot_EGF0min_vs_15min, width = 10, height = 8, scale = 0.4)
# 
# ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/my_isoform_volcano_plot_EGF0min_vs_15min.pdf", plot = my_isoform_volcano_plot_EGF0min_vs_15min, width = 10, height = 8, scale = 0.4)
...
isoform_diff_abundance_gene <- isoform_diff_abundance %>%
  ...
  mutate(comparison = case_when(
    ...
    TRUE ~ comparison)) %>% 
  mutate(
    reference = str_sub(reference_isoform_cluster, end = 6L),
    isoform = str_sub(reference_isoform_cluster, start = 7L),
    gene_isoform = paste(gene, isoform, sep = " "),
    num_mod_res = str_count(isoform, "[STY]"),
    mod_res = str_remove_all(isoform, "[0-9]"))
...
my_isoform_volcano_plot_EGF15min_vs_0min_STYcolor <- ggplot() +
  geom_text_repel(data = isoform_diff_abundance_gene %>%
               ...
               filter(diff >= 1) %>%
               filter(adj_pval <= 0.05),
               mapping = aes(x = diff, y = -log10(adj_pval), label = gene_isoform),
               max.overlaps = 200,
  ...
  geom_point(data = isoform_diff_abundance_gene %>%
               ...
               # filter(abs(diff) < 1) %>% 
               filter(adj_pval >0.05),
             mapping = aes(x = diff, y = -log10(adj_pval)), color = "grey", alpha = 0.3, shape =21) +
  geom_point(data = isoform_diff_abundance_gene %>%
               filter(comparison == "EGF15min_vs_EGF0min") %>% 
               filter(abs(diff) < 1) %>%
               filter(adj_pval <0.05),
  ...
  geom_point(data = isoform_diff_abundance_gene %>%
               ...
               filter(diff < -1) %>% 
               filter(adj_pval <= 0.05),
             mapping = aes(x = diff, y = -log10(adj_pval), fill = mod_res), color = "black", alpha = 0.3, shape =21, size = 1) +
  geom_point(data = isoform_diff_abundance_gene %>%
               filter(comparison == "EGF15min_vs_EGF0min") %>% 
               filter(diff > 1) %>% 
               filter(adj_pval <= 0.05),
  ...
  scale_x_continuous(name = "Log2(fold change)", breaks = seq(-6, 12, 2), limits = c(-6, 12)) +
  scale_y_continuous(name = "-Log10(adj. p-value)", breaks = seq(0,36,4), limits = c(0, 36))

my_isoform_volcano_plot_EGF15min_vs_0min_STYcolor

ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/my_isoform_volcano_plot_EGF15min_vs_0min_STYcolor.png", plot = my_isoform_volcano_plot_EGF15min_vs_0min_STYcolor, width = 10, height = 12, scale = 0.4)

ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/my_isoform_volcano_plot_EGF15min_vs_0min_STYcolor.pdf", plot = my_isoform_volcano_plot_EGF15min_vs_0min_STYcolor, width = 15, height = 10, scale = 0.4)


## --------------------------------------------------------------------------------------------------------------------------------------------------
# EGF5min vs. EGF0min (aka control/ untreated) ------------------------------------------------------

my_isoform_volcano_plot_EGF5min_vs_EGF0min_STYcolor <- ggplot() +
  geom_vline(xintercept = c(-1, 1), color = "darkgrey", alpha = 0.5, size =1, linetype = 2)+
  geom_hline(yintercept = -log10(0.05), color = "darkgrey", alpha = 0.5, size = 1, linetype = 2) +
  geom_point(data = isoform_diff_abundance_gene %>%
...
my_isoform_volcano_plot_EGF5min_vs_EGF0min_STYcolor

ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/my_isoform_volcano_plot_EGF5min_vs_EGF0min_STYcolor.png", plot = my_isoform_volcano_plot_EGF5min_vs_EGF0min_STYcolor, width = 15, height = 10, scale = 0.4)

ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/my_isoform_volcano_plot_EGF5min_vs_EGF0min_STYcolor.pdf", plot = my_isoform_volcano_plot_EGF5min_vs_EGF0min_STYcolor, width = 15, height = 10, scale = 0.4)
...
ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/my_isoform_volcano_plot_EGF3min_vs_0min_STYcolor.png", plot = my_isoform_volcano_plot_EGF3min_vs_0min_STYcolor, width = 15, height = 10, scale = 0.4)

ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/my_isoform_volcano_plot_EGF3min_vs_0min_STYcolor.pdf", plot = my_isoform_volcano_plot_EGF3min_vs_0min_STYcolor, width = 15, height = 10, scale = 0.4)
...
ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/my_isoform_volcano_plot_EGF1min_vs_0min_STYcolor.png", plot = my_isoform_volcano_plot_EGF1min_vs_0min_STYcolor, width = 15, height = 10, scale = 0.4)

ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/my_isoform_volcano_plot_EGF1min_vs_0min_STYcolor.pdf", plot = my_isoform_volcano_plot_EGF1min_vs_0min_STYcolor, width = 15, height = 10, scale = 0.4)
...
#                               diff_EGF0min_vs_EGF1min = c(-6, 6, -6, 6, -6, 6),
#                               diff_EGF0min_vs_EGF3min = c(-6, 6, -6, 6, -6, 6),
#                               diff_EGF0min_vs_EGF5min = c(-6, 6, -6, 6, -6, 6),
...
ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/signif_change_isoform_cluster_heatmap.png", plot = signif_change_pprotein_heatmap, height = 40, width = 10, scale = 0.4)
...
pheatmap_diff_pval_df_all <- isoform_diff_abundance_gene %>%
  ...
  #change NA to 0:
  mutate(
    diff_EGF0min_vs_EGF1min = ifelse(is.na(diff_EGF1min_vs_EGF0min), 0, diff_EGF1min_vs_EGF0min),
    diff_EGF0min_vs_EGF3min = ifelse(is.na(diff_EGF3min_vs_EGF0min), 0, diff_EGF3min_vs_EGF0min),
    diff_EGF0min_vs_EGF5min = ifelse(is.na(diff_EGF5min_vs_EGF0min), 0, diff_EGF5min_vs_EGF0min),
...
#                                    border_color = "black" )
# 
# ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/signif_change_pprotein_heatmap_all_isoforms1001-1500.png", plot = signif_change_pprotein_heatmap_all, height = 80, width = 6, scale = 0.4)
...
# 
#   filter(comparison %in% c("EGF15min_vs_EGF0min", "EGF5min_vs_EGF0min", "EGF0min_vs_EGF3min", "EGF0min_vs_EGF1min")) %>%
#   mutate(
#     isoform_cluster = str_sub(reference_isoform_cluster, start = 8L),
#     gene_isoform = paste(gene, isoform_cluster, sep = " "),
#     condition = as.factor(condition),
...
## --------------------------------------------------------------------------------------------------------------------------------------------------
# write_csv(x = ascore_final_gene %>% mutate(gene_ref = paste(gene, isoform_cluster, sep = "_")), file = "modified_data/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/isoform_SHINY_input.csv")


## --------------------------------------------------------------------------------------------------------------------------------------------------
most_common_protein_family_per_site <- read_csv("modified_data/MainFig3to6_SuppFig9to15/pTyrSiteLevel/KEY_most_common_protein_family_per_site.csv")


## --------------------------------------------------------------------------------------------------------------------------------------------------
SHINY_pY_isoforms <- ascore_final_gene %>%
  mutate(gene_ref = paste(gene, isoform_cluster, sep = "_")) %>%
  separate(isoform_cluster, into = c("site1", "site2", "site3"), sep = " ", remove = FALSE) %>% 
...
write_csv(x = SHINY_pY_isoforms, file = "modified_data/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/pY_ISOFORM_input.csv")
...
ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/pY_boxplot_points_EGFR.png", plot = pY_boxplot_points, height = 12, width = 20, scale = 0.4)
ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/pY_boxplot_points_EGFR.pdf", plot = pY_boxplot_points, height = 12, width = 20, scale = 0.4)
...
ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/pY_boxplot_points_MAPK1.png", plot = pY_boxplot_points_MAPK1, height = 8, width = 5, scale = 0.4)


## --------------------------------------------------------------------------------------------------------------------------------------------------
# boxplot_gene_psite_function <- function(vector_gene_refs, dataframe, output_dir = "output/DDA_YP/EGF_stim/", condition = condition, sample_id = sample_id){
# 
...
## --------------------------------------------------------------------------------------------------------------------------------------------------
# boxplot_gene_psite_function <- function(vector_gene_refs, dataframe, output_dir = "output/DDA_YP/EGF_stim/noPOOL/isoform/", condition = condition, sample_id = sample_id, strip_color = "gray80", strip_text_color = "black"){
# 
#   #create output directory if it doesn't exist already
#   if (!dir.exists(output_dir)){
#     dir.create(output_dir)}
...
#           axis.text.x  = element_text(angle = 0, family = "sans", size = 12),
...
#       
#       # save the plot as PNG & PDF
#       ggsave(filename = file.path(output_dir, paste0(gene_var, "_boxplot_points1.png")), plot = p, width = 7, height = 7, scale = 0.4)
#       ggsave(filename = file.path(output_dir, paste0(gene_var, "_boxplot_points1.pdf")), plot = p, width = 7, height = 7, scale = 0.4)
#       
#     } else { 
#       message (paste("Gene", gene_var, "not found in the dataframe."))
#       
...
## --------------------------------------------------------------------------------------------------------------------------------------------------
# boxplot_gene_psite_function(vector_gene_refs = (ascore_final_gene %>% distinct(gene_ref))$gene_ref, dataframe = ascore_final_gene %>% distinct(gene, gene_ref, condition, replicate, sample_id, median_norm_intensity, n_obs) %>% mutate(condition = fct_relevel(condition, "EGF0min", "EGF1min", "EGF3min", "EGF5min", "EGF15min")), output_dir = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/boxplot_points_scaled/")


## --------------------------------------------------------------------------------------------------------------------------------------------------

# vector_gene_refs = ((ascore_final_gene %>% distinct(gene_ref) %>% filter(gene_ref == "GAGE2E Y17")))
# 
# 
# boxplot_gene_psite_function(vector_gene_refs = vector_gene_refs,
#   # vector_gene_refs = (ascore_final_gene %>% distinct(gene_ref))$gene_ref,
#                             dataframe = ascore_final_gene %>%
#                               distinct(gene, gene_ref, condition, replicate, sample_id, median_norm_intensity, n_obs) %>%
#                               mutate(condition = fct_relevel(condition, "EGF0min", "EGF1min", "EGF3min", "EGF5min", "EGF15min")),
#                             strip_text_color = "black",
#                             strip_color = "grey",
#                             output_dir = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/")
...
ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/all_isoform_PC_1_2.png", plot = all_isoform_PC_1_2, width = 24, height = 12, scale = 0.4)
...
ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/all_isoform_PC_1_3.png", plot = all_isoform_PC_1_3, width = 24, height = 12, scale = 0.4)
...
# 
# ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/literature_PTMSEA_EGFR_pathway_modres_distribution.png", plot = plot_PTMSEA_EGFR_pathway_modres, width = 6, height = 6, scale = 0.4)
...
# 
# 
# ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/EGFR1_isoform_overlaps_pY.png", plot = EGFR1_isoform_overlaps,
...
ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/EGFR1_overlaps_protein.png", plot = EGFR1_overlaps_protein,
...
#   ylab("unqiue p-isoforms")
# EGFR1_isoform_overlaps
# ggsave(filename = "output/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/EGFR1_isoform_overlaps.png", plot = EGFR1_isoform_overlaps,
#        scale = 0.4, width = 6, height = 8)
# 
# #plot
# plot_intensity_pisoforms_EGFR1 <- ggplot(data = EGFR1_overlaps) +
...
#   filter(comparison %in% c("EGF0min_vs_EGF15min", "EGF5min_vs_EGF0min", "EGF0min_vs_EGF3min", "EGF0min_vs_EGF1min")) %>%
#   
#   #flip the 0 min and 15 min comparison
#   mutate(diff = case_when(
#     comparison == "EGF0min_vs_EGF15min" ~ -diff,
...
# 
# write_csv(x = PTMNav_isoform_input, file = "modified_data/MainFig3to6_SuppFig9to15/pTyrIsoformLevel/PTMNav_isoform_input.csv")
